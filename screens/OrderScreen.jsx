...
const OrderScreen = ({ route }) => {
  ...
  const fetchSubscription = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `${process.env.EXPO_PUBLIC_API_URL}/order/get?id=${orderId}`,
        {
          headers: {
  ...
  const getSkippedDatesForCurrentMonth = () => {
    ...
    return subscription.skippedDates
      .map((d) => new Date(d))
      .filter(
        (d) => d.getFullYear() === currentYear && d.getMonth() === monthIndex
      )
      .map((d) => d.getDate())
      .sort((a, b) => a - b);
  };

  const skippedDatesForMonth = getSkippedDatesForCurrentMonth();
      ...
      >
        <View style={styles.content}>
          ...
          <View style={styles.dateInfoContainer}>
            <View>
              <Text style={styles.dateLabel}>Order Created At:</Text>
